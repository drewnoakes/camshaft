...
TEST(UrlTest, parseQueryString_splitCsv)
{
  auto res = parseQueryString("a=1,2,3&b=a&b=b,c", true);
  ...
  ASSERT_EQ(3, distance(a_range.first, a_range.second));
  ASSERT_EQ(3, distance(b_range.first, b_range.second));
  ...
  ASSERT_EQ("1", a_values[0]);
  ASSERT_EQ("2", a_values[1]);
  ASSERT_EQ("3", a_values[2]);

  ASSERT_EQ("a", b_values[0]);
  ASSERT_EQ("b", b_values[1]);
  ASSERT_EQ("c", b_values[2]);
}
